...
export const POD_LEAD_ORDER = ["Paul", "Josh", "Nishant", "Dan", "Jacob"];
...
// ─── Dashboard Overrides (read/write via Sheets v4 API) ───

const OVERRIDES_TAB_NAME = "Dashboard Overrides";

export async function fetchDashboardOverrides() {
  const { createGoogleSheetsClient } = await import("./google-service.js");
  ...
  try {
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: `'${OVERRIDES_TAB_NAME}'!A:C`,
    });

    const rows = response.data.values || [];
    const overrides = new Map();
    for (let i = 1; i < rows.length; i++) {
      const adCode = String(rows[i][0] || "").trim();
      const classification = String(rows[i][1] || "").trim().toLowerCase();
      if (adCode && (classification === "this_week" || classification === "spillover")) {
        overrides.set(adCode.toLowerCase(), classification);
      }
    }
    return overrides;
  } catch {
...
export async function writeDashboardOverride(adCode, classification) {
  const { createGoogleSheetsWriteClient } = await import("./google-service.js");
  ...
  try {
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `'${OVERRIDES_TAB_NAME}'!A1:C1`,
      valueInputOption: "RAW",
      requestBody: { values: [["Ad Code", "Classification", "Timestamp"]] },
    });
    existingRows = [["Ad Code", "Classification", "Timestamp"]];
  }

  const normalizedAdCode = adCode.trim();
  let rowIndex = -1;
  for (let i = 1; i < existingRows.length; i++) {
    if (String(existingRows[i][0] || "").trim().toLowerCase() === normalizedAdCode.toLowerCase()) {
      rowIndex = i;
      break;
    }
  }
  ...
  if (rowIndex >= 0) {
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `'${OVERRIDES_TAB_NAME}'!A${rowIndex + 1}:C${rowIndex + 1}`,
      valueInputOption: "RAW",
      requestBody: { values: [[normalizedAdCode, classification, timestamp]] },
    });
  } else {
  ...
  return { ok: true, adCode: normalizedAdCode, classification, timestamp };
}

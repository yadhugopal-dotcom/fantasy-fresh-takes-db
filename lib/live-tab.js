...
const CSV_FETCH_TIMEOUT_MS = 10000;
const SHEETS_API_TIMEOUT_MS = 15000;
...
function writeSupabaseSheetCache(sheetName, rows) {
  // Non-blocking: fire and forget
  writeJsonObject(sheetCachePath(sheetName), { rows, cachedAt: Date.now(), sheetName }).catch(() => {});
}
...
async function fetchTrackerSheetRows(sheetName) {
  ...
  for (const attempt of fetchAttempts) {
    try {
      ...
      writeSupabaseSheetCache(sheetName, parsedRows); // persist for next cold start
